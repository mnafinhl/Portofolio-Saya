...
class FaceMesh():
    ...
    def masking(self, image, filter_img, face_landmarks, face_part, INDEXES):
        ...
        try:
            ...
            # Bikin mask
            gray = cv2.cvtColor(resized_filter_img, cv2.COLOR_BGR2GRAY)
            ...
            if face_part == 'MOUTH':
                ...
                location = (int(center[0] - filter_img_height/2), int(center[1]-filter_img_height/2))

            x = max(0, location[0])
            y = max(0, location[1])

            ROI = annotated_image[y: y + filter_img_height, x: x + filter_img_width]

            # Gabungkan
            bg = cv2.bitwise_and(ROI, ROI, mask=mask)
            fg = cv2.bitwise_and(resized_filter_img, resized_filter_img, mask=cv2.bitwise_not(mask))
            dst = cv2.add(bg, fg)

            annotated_image[y: y + filter_img_height, x: x + filter_img_width] = dst

        except Exception as e:
            print("Error in masking:", e)
