...
cap.set(3,1280)
cap.set(4,960)
left_eye = cv2.imread(r'D:\OSN INFORMATIKA\Phyton AI\Face Filter Dragon\assets\eye1.png')
right_eye = cv2.imread(r'D:\OSN INFORMATIKA\Phyton AI\Face Filter Dragon\assets\eye2.png')
print("Left Eye:", left_eye is not None)
print("Right Eye:", right_eye is not None)
smoke_animation = cv2.VideoCapture(r'D:\OSN INFORMATIKA\Phyton AI\Face Filter Dragon\assets\smoke_animation.mp4')
...
while True:
    ...
    # print("Face Detected:", face_mesh_results.multi_face_landmarks)
        ...
        for face_num, face_landmarks in enumerate(face_mesh_results.multi_face_landmarks):
            if left_eye_status[face_num] == 'OPEN':
                frame = detector.masking(frame, left_eye,
                                         face_landmarks, 'LEFT EYE', detector.mpFaceMesh.FACEMESH_LEFT_EYE)
                if right_eye_status[face_num] == 'OPEN':
                    frame = detector.masking(frame, right_eye,
                                         face_landmarks, 'RIGHT EYE', detector.mpFaceMesh.FACEMESH_RIGHT_EYE)
